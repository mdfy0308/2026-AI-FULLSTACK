  ...
  <form action="${pageContext.request.contextPath}/security/join" method="post" onsubmit="return checkForm()">
    ...
    window.addEventListener("load", function(){
		...
		nickname.addEventListener("keyup", function( e ){
			...
			if(value !== ""){ 
				fetch("${pageContext.request.contextPath}/doubleName?nickname=" + encodeURIComponent(value))
				.then( res => res.json() )
				.then( data => { 
					if(data.exists){
						target.textContent="이미 사용중인 닉네임입니다.";
						target.className = "my-3 alert alert-danger tnickname"
					} else { 
						target.textContent="사용 가능한 닉네임입니다.";
						target.className = "my-3 alert alert-success tnickname"
					}
				})
				.catch( err => { 
					target.textContent="서버 오류";
					target.className = "my-3 alert alert-info tnickname"
				});						
				
			} 
			else { 
				target.textContent="닉네임 중복검사는 필수입니다.";
				target.className = "my-3  alert  alert-warning  tnickname"
			} 
		});
	});
    </script>
