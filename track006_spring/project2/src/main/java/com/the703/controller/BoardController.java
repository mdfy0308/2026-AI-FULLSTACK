...
public class BoardController {  
   ...
   //테스트 : http://localhost:8080/project2/board/list.do
   ...
   //■2. 글쓰기 기능 
   //@PreAuthorize(hasRole('ROLE_ADMIN'), hasRole('ROLE_MEMBER')) // 권한들 중 하나
   //@PreAuthorize("isAuthenticated() and hasRole('ROLE_ADMIN')") // 로그인+ADMIN 권한
   //@PreAuthorize("isAuthenticated()") // 로그인을 했다면
   @PreAuthorize("isAnonymous()") // 로그인하지않아도 글쓰기 가능
