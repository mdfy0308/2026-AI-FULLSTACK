...
2. public ip
    > 13.125.236.249
3. ssh 클라이언트
    > ssh -i "the703-sj.pem" ubuntu@ec2-13-125-236-249.ap-northeast-2.compute.amazonaws.com

※ the703-sj.pem → 보관주의(개인이 보관할 것)
```bash
chmod 400 "the703-sj.pem" # 소유자(4:r4w2x1 읽기 가능) 그룹(0) 다른사람(0)
```

- ctrl + `
```bash
# 1. 상속 권한 완전히 제거
icacls "the703-sj.pem" /inheritance:r /grant:r "$($env:USERNAME):(R)"

# 2. 혹시 남아있을 수 있는 다른 사용자 권한 강제 삭제
icacls "the703-sj.pem" /remove "NT AUTHORITY\Authenticated Users"
icacls "the703-sj.pem" /remove "BUILTIN\Users"
icacls "the703-sj.pem" /remove "NT AUTHORITY\SYSTEM"
```
...
4. ECR 리포지토리
- 애플리케이션을 docker 이미지로 빌드해서 ecr에 올려두면 어디서든지 가져다가 사용 가능하게

※ ECR 검색
1) 리포지토리 이름
2) 이미지 태그 설정  - Mutable(연습-latest 덮어쓰기 가능)
3) 암호화 설정 - 기본키 그대로
```
718875642256.dkr.ecr.ap-northeast-2.amazonaws.com/the703-js
```
...
#    사용자계정     시스템그룹     docker
# 현재 로그인한 사용자에게 docker 그룹 권한을 줘서 sudo 없이 docker명령어 사용 가능하게
```
...
8) 실행 디렉토리 생성(★)
```bash
#1. 상위 폴더 및 uploads 폴더까지 한번에(-p) 생성(mkdir)
mkdir -p /home/ubuntu/app/back/build/libs/uploads

#2. 홈 프로젝트 기본 디렉토리 권한 설정 (소유자는 모든 권한 7 그룹/다른 사람은 읽기+실행 5)
sudo chmod 755 /home/ubuntu
sudo chmod 755 /home/ubuntu/app
sudo chmod 755 /home/ubuntu/app/back/build/libs/uploads
sudo chmod 644 /home/ubuntu/app/back/build/libs/uploads/* # 이미 생성된 것이 있다면
```
...
# sudo follocate -l 2G /swqpfile ← 2GB 파일 생성
# sudo chmod 600 /swapfile ← 권한 유저(r읽기 w쓰기 x실행)
...
```bash
        ...
        # 2. system 계정으로 접속 (비밀번호 변경 반영) / ctrl+c로 빠져나오기
...
CI : 지속적 통합
- 공용 저장소에 자주 병합
- 자동 빌드/테스틀을 통해서 조기에 버그 발견

CD : 지속적 제공/배포
- 자동으로 프로덕션 환경에 배포

1. git repository 새로 만들기
    > https://github.com/mdfy0308/track009_aws.git

2. Actions secrets and variables 시크릿 키 설정

```bash
Name: EC2_HOST
Secret: 13.125.236.249
# 현재 EC2 퍼블릭 IP
...
git clone https://github.com/mdfy0308/track009_aws.git
...
> 해결방안1) 프로젝트가 망가졌다면 - 올리다가 망가졌다!
1. boot 프로젝트 새로 만들기 - com.thejoa703
2. .env, build.gradle 옮기기
3. src 안에 덮기

> 해결방안2) 500 에러가 나타난 경우
```bash
1) ssh 접속
ssh -i "the703-sj.pem" ubuntu@ec2-13-125-236-249.ap-northeast-2.compute.amazonaws.com
2) pm2 list
