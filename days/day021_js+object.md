...
1. js 선택자 
1-1. document.querySelector("css표현방법")과 document.querySelectorAll("css표현방법")의 차이?
1-2. 아래 요구사항에 맞게 코드를 작성하세요.
...
<input type="button"  value="addEventListener"  title="버튼4"  id="test"    class="btn btn-outline-primary"  />  
  <script>
    // 2-1. HTML 문서가 모두 로드된 후 실행되도록 이벤트 등록
    window.addEventListener("load", function() {
      
    // 2-2. 아이디가 test인 요소를 querySelector로 선택하여 변수에 저장
    let test = document.querySelector("#test");

    // 2-3. 해당 요소를 클릭했을 때 prompt로 좋아하는 색상 입력받기
    test.onclick = function() {
      let color = prompt("좋아하는 색상을 입력하세요 (예: red, blue, green)");

    // 2-4. 입력받은 색상을 버튼 배경색으로 적용
