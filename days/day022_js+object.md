...
Book b2 = new Book();
b2.setTitle("Spring"); // 메인에서 일어나는 일
```
...
상속은 기존 클래스의 (㉕ 속성(상태, 멤버변수) )와 기능을 재사용하기 위해 사용된다.
...
오버로딩은 같은 클래스 내에서 (㉙ 이름 )이 같고 (㉚ 파라미터 )가 다른 메서드를 정의하는 것.
오버라이딩은 부모 클래스의 메서드를 자식 클래스에서 (㉛ @Override )하여 재정의하는 것.
오버로딩은 (㉜ 컴파일 ) 시점에 결정되고, 오버라이딩은 (㉝ 실행 ) 시점에 결정된다.
오버라이딩 시 접근 제어자는 부모 메서드보다 (㉞ 넓  ) 범위로만 변경 가능하다.