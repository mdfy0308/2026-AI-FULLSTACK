...
// 모바일 기기에서 갤러리/카메라 등 접근 라이브러리
import 'package:image_picker/image_picker.dart';

// 서버 BASE URL : 10.0.0.2 / localhost:8080 / http://~
...
class BoardNotifier extends Notifier<BoardState> {
  @override
  BoardState build() {
    _initDio(); // _는 private 속성
    return const BoardState();
  }

  late final Dio _dio; // late - 지연 초기화 : Dio 객체 초기화
  final _storage = const FlutterSecureStorage(); // 보안 저장소

  // 설정
  void _initDio() { // 기본옵션 설정
    _dio = Dio(BaseOptions(
      baseUrl: ApiClient.getBaseUrl(), // 10.0.0.2 / localhost:8080 / http://~
    ));
    ...
    _dio.interceptors.add( InterceptorsWrapper ( //## InterceptorsWrapper
      ...
      onRequest: (options, handler) async {
        final token = await _storage.read(key: 'accessToken'); // 토큰 읽기
        if (token != null) {
  ...
  Future<bool> _refreshAccessToken() async {
    try {
      ...
      final newAccessToken = response.data['accessToken']; // map Token 추출
  ...
  Future<bool> createPost({
    ...
    try {
      ...
      if (imageFiles != null && imageFiles.isNotEmpty) { // 첨부 이미지가 있는 경우
      ...
      await _dio.post('/api/posts', data: formData); // MultiPartFile - post 요청
      await fetchPosts(); // 글 다 쓰고 게시글 목록 새로고침
      return true;
...
final boardProvider = NotifierProvider<BoardNotifier, BoardState>(() { // ## 1.  전역 provider
...
class _PostListPageState extends ConsumerState<PostListPage> {
  ...
  String _resolveImageUrl(String url) {
    ...
    final String serverBaseUrl = ApiClient.getBaseUrl(); // http://서버도메인.org ,  localhost:8080
  ...
  @override
  void initState() { //  위젯 연결시 1번
    super.initState();
    // 화면에 그림 그리기
    Future.microtask(() => ref.read(boardProvider.notifier).fetchPosts());
  } // ## react useEffect( ./..., [] ), 1번 읽어들임 ## read

  @override
  Widget build(BuildContext context) {
    final boardState = ref.watch(boardProvider); // ## watch 지속적인 확인 useEffect( ./..., [user] )
    final authState = ref.watch(authProvider); // 로그인 상태 감지

    return AppLayout( // ## 공통 레이아웃
      child: Scaffold(
        body: boardState.loading && boardState.posts.isEmpty // 조건 1: 로딩중?
            ? const Center(child: CircularProgressIndicator()) // 로딩화면
            : boardState.posts.isEmpty // 조건 2: 비어있다면
                ? const Center(child: Text('등록된 게시글이 없습니다.'))
                : RefreshIndicator( // ## 화면 당겨서 새로고침 - RefreshIndicator
                    onRefresh: () async {
                      await ref.read(boardProvider.notifier).fetchPosts(); // 게시글 가져와주는 api
                    },
                      ...
                      itemBuilder: (context, index) {
                        ...
                        return Card(
                          margin: const EdgeInsets.symmetric(
                              horizontal: 12, vertical: 6), // 카드 외각 마진 설정
                          child: InkWell(
                            onTap: () {
                              Navigator.push(
                                context,
                                  ...
                                  if (imageUrls.isNotEmpty) ...[
                                    ...
                                    ClipRRect( // 자식 이미지 모서리 둥글게
                                      ...
                                      child: Image.network(
                                        ...
                                        height: 120,
                                        width: double.infinity,
                                        fit: BoxFit.cover, // 이미지 비율 꽉 채우기(odject-fit : cover)
                                        errorBuilder: // 이미지 로드 실패시
        ...
        floatingActionButton: FloatingActionButton( // 화면 우측 하단 플로팅 글쓰기 버튼 
          onPressed: () { // ## 버튼 클릭(탭)
            if (authState.user == null && authState.accessToken == null) {
              ...
              Navigator.pushNamed(context, '/login'); // pushNamed 화면 이동
            } else {
              Navigator.pushNamed(context, '/post-write');
            }
          },
          child: const Icon(Icons.create), // 연필 아이콘
        ),
      ),
    );
  }
...
```bash
flutter pub get
dart run flutter_launcher_icons # 아이콘 생성
