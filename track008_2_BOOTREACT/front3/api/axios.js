...
const api = axios.create({
    // 기본 api 서버주소, 환경 변수 없으면 로컬서버 사용
  baseURL: process.env.NEXT_PUBLIC_API_BASE_URL || "http://localhost:8080",
    // refreshToken이 HttpOnly 쿠키에 저장되어 있으면 자동으로 포함 필요
...
// 요청 인터셉터 : 요청 보내기 전에 AccessTiken을 헤더에 추가
  ...
  async (error) => {
    ...
    if (status === 401 && !original._retry) {
      ...
      try {
        ...
        const newAccessToken = data?.accessToken; // Access Token
