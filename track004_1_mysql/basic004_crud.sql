...
-- 10:35:42	insert into userinfo values ('aaa', 11, 2)	
-- Error Code: 1366. Incorrect integer value: 'aaa' for column 'no' at row 1 

-- insert into userinfo values (1, 'bbb', 22);
-- 10:37:17	insert into userinfo values (1, 'bbb', 22)	
...
-- 10:41:25	insert into userinfo (name, age) values (22, 'bbb')	Error Code: 1366. 
-- Incorrect integer value: 'bbb' for column 'age' at row 1	0.000 sec
-- 원하는 순서를 지정하여 데이터를 삽입하는 것도 가능합니다.
-- 필드 순서를 정했다면 자료형에 맞게 데이터도 순서대로 삽입할 것
...
SET SQL_SAFE_UPDATES = 0; -- safe update를 사용하지 않음 / 

use mbasic;
show tables;
delete from userinfo; -- 데이터 삭제
-- 11:43:20	delete from userinfo Error Code: 1175. 
...
insert into userinfo(name, age) values ('third', 33);
...
-- update userinfo set name = 'first' 			 where no = 4; -- 여기서 first라고 수정해주지 않으면 조건 충족 안 됨
update userinfo set name = 'fourth', age = 44 where no = 4 and name = 'first'; -- 조건에 안 맞으면 업데이트 안 됨
...
insert into score(sname, sjava, sjsp, sspring) values ('aaa', 100, 100, 100, 100);
insert into score(sname, sjava, sjsp, sspring) values ('bbb', 90, 98, 88, 100);
insert into score(sname, sjava, sjsp, sspring) values ('ccc', 70, 20, 78, 80);
insert into score(sname, sjava, sjsp, sspring) values ('ddd', 78, 89, 68, 98);
insert into score(sname, sjava, sjsp, sspring) values ('abc', 89, 98, 69, 77);
...
create table userinfo2 select * from userinfo; -- 구조+데이터 복사
-- no에 PK, auto_increment 추가
