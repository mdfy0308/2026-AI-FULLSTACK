...
select ban, count(*) -- 6(집계 안 된 일반), 1(집계)
from group_userinfo ;
-- 10:42:24	select ban, count(*)  
-- from group_userinfo LIMIT 0, 1000	Error Code: 1140. In aggregated query without GROUP BY, 
-- expression #1 of SELECT list contains nonaggregated column 'mbasic.group_userinfo.ban'; 
-- this is incompatible with sql_mode=only_full_group_by	0.000 sec

select 		ban, count(*) -- 반, 갯수
from 		group_userinfo
group by 	ban; -- 반 별 (집계 필드 빼고 다)
...
select ban, count(if(age < 20, 1, null)) `미성년자수`
from group_userinfo
group by ban;
...
select ban, sum(sns='n') `sns수신거부수`
from group_userinfo
group by ban;

select ban, count(if(sns = 'n', 1, null)) `sns수신거부수`
from group_userinfo
group by ban;
...
select ban, sum((kor+eng+math)/3) `평균 총점`
from group_userinfo
group by ban
...
select ban, max((kor+eng+math)/3) `평균 점수`
from group_userinfo
group by ban
order by max((kor+eng+math)/3) desc
limit 1;
...
select ban, sum(sex='m') `남학생수`, sum(sex='f') `여학생수`
from group_userinfo
group by ban;

select ban, count(if(sex='m', 1, null)) `남학생수`, count(if(sex='f', 1, null)) `여학생수`
from group_userinfo
group by ban;
...
select count(*) `부서 30 직원수`
from emp
where deptno = 30; -- 집계 전에 구분

select count(*) `부서 30 직원수`
from emp
group by deptno
having deptno = 30; -- 집계 후에 구분
...
from emp
where deptno = 10; -- 집계전 필터링

select max(sal) `부서 10 최대급여`
from emp
group by deptno
having deptno = 10; -- 집계 후 필터링
...
group by deptno, job
order by deptno, job;
-- 11:41:26	select deptno, job, avg(sal) from emp where avg(sal) >= 2000 group by deptno, job order by deptno, job LIMIT 0, 1000
-- Error Code: 1111. Invalid use of group function	0.000 sec
-- 집계 함수를 GROUP BY와 함께 쓸 때, 집계 함수가 허용되지 않는 절(주로 WHERE)에서 조건을 지정했을 때 발생
-- having을 사용하자
