...
create table userinfo_select select * from userinfo where 1=2; -- 구조복사 1=2 같니? 해당하는 값 X 구조만 복사alter
...
insert into userinfo_select(name, age) values ('fourth', 44);
...
select 1.2345, 
	   round(12.2545, 1), -- round(데이터, .을 기준으로한 위치에서 반올림) 소수점 1번째 자리에서 반올림 12.3
       round(15.2345, -1) -- < (-)   0.0   (+) > 정수 1번째 자리에서 반올림 20
from   dual; 
...
select 123.4578, 
	   round(123.4578, 0) `Round 1`,
       round(123.4578, -1) `Round 2`,
       round(123.4578, 1) `Round 3`,
       round(123.4578, 2) `Round 4`
from dual;
...
select instr('abc', 'b')  `abc에서 b의 위치`,
...
select substr('abcde', 2, 2) `s1`, -- substr(문자열, 시작, 몇 개)
	   substr('abcde', 2, 3) `s2`,
	   substr('abcde', 1, 3) `s3`
from dual;
...
select concat('#', trim(' a b c '), '#') `양쪽 공백 제거`,
	   concat('#', ltrim(' a b c '), '#') `왼쪽 공백 제거`,
       concat('#', rtrim(' a b c '), '#') `오른쪽 공백 제거`
from dual;
...
select replace('hello alpha!', 'alpha', 'beta') `찾아서 바꾸기`
from dual;
...
select lpad('ABC', 5, '#') `왼쪽 공백 채우기` ,  
	   rpad('ABC', 5, '#') `오른쪽 공백 채우기`
from dual;
...
alter table fn_select_userinfo2 drop comm;
...
update fn_select_userinfo2 set email = 'aaa@gmail.com' where no=1;
update fn_select_userinfo2 set email = 'bbb@gmail.com' where no=2;
update fn_select_userinfo2 set email = 'ccc@gmail.com' where no=3;
update fn_select_userinfo2 set email = 'ddd@gmail.com' where no=4;
update fn_select_userinfo2 set email = 'abc@gmail.com' where no=5;
update fn_select_userinfo2 set email = 'bca@gmail.com' where no=6;
...
-- |    66     |
-- +-----------+
select max(age) `최대 나이`
from fn_select_userinfo2
where instr(email, 'gmail.com');
...
select round(avg(age), 1) `30대 이상 평균나이`
from fn_select_userinfo2
where age >= 30;
...
-- WHERE age = (SELECT MAX(age) FROM fn_select_userinfo);
...
select name, email
from fn_select_userinfo2
where length(name) = (select max(length(name)) from fn_select_userinfo2);
