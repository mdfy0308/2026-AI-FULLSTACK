...
```bash
# pip : Node.js의 npm, Java Gradle/Maven 같은 패키지 설치 관리자
...
```bash
# 장고 프로젝트는 여러 개의 App(기능단위 모듈)으로 구성
# analytics 앱 생성
# python manage.py startapp 앱이름(게시판, 장바구니 등)
...
│   └── templates/     <-- [화면] 템플릿 폴더(html, css...)
│       └── analytics/
│           └── dashboard.html <-- [화면] 대시보드 시각화 HTML
├── board/             <-- (추가한다면 이런 방식으로) 게시판 앱 폴더
│
...
```python
#DB에 생성할 데이터 구조를 class로 작성(JPA Entity 선언 동일)
...
```bash
# 1. 마이그레이션 생성 - models.py 변경사항으로 db 설계도를 생성
...
## M(model) T(html, css:화면) V(Controller: 비즈니스 로직, DB와 View 연결)
...
def dashboard_view(request):
    qs = ServiceLog.objects.all().values('date', 'category', 'visitot_count', 'sales_amount')
    ...
    if qs.exists():
        ...
        #34495e       1) 그룹핑     합계      재정렬
        summary_df = df.groupby('category')[['visitot_count', 'sales_amount']].sum().reset_index()

        # 3. 템플릿 전달용 순수 파이썬 리스트 추출
        categories = summary_df['category'].tolist()
        cisitors = summary_df['visitot_count'].tolist()
...
urlpatterns = [
    ...
    # 기본 경로, 해결사: 처리.dashboard_view, 이름
...
urlpatterns = [
    ...
    # /dashboard/ 주소로 들어올 경우
    path('dashboard/', include('analytics.urls')),
    # http://127.0.0.1:8000 접속시, /dashboard/로 자동 이동
