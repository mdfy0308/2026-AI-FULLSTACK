...
function* watchSignUp(){
    ...
    // SIGN_UP_REQUEST 액션 발생 → 여러번 요청시 가장 마지막 요청만 처리
}

// ----------------------------- 사용자 조회    watchLoadUsers
//get  : /user/
export function loadUsersApi(){
    return client.get('/user'); // http://localhost:3065/user/login
}
...
export function updateNicknameApi(data){
    return client.patch(`/user/${data.id}/nickname`, {nickname: data.nickname}); // http://localhost:3065/user/login
}
...
export function deleteUserApi(id){
    return client.delete(`/user/${id}`); // http://localhost:3065/user/logout
}
